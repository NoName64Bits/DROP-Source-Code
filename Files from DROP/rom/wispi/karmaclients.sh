#!/bin/sh
iw dev wlan0 station dump > /tmp/stadump #dump all stations connected

function GetHosts() {
local i=1
sta_list=$(cat /tmp/stadump | grep -e "Station" | cut -f 2 -d" ") #list all station bssid
sta_count=$(cat /tmp/stadump | grep -e "Station" | cut -f 2 -d" " | wc -w) #count how many stations
	...
	sta_linenum[${i}]=$(grep -n -E "${sta[${i}]}" /tmp/stadump | cut -f1 -d: ) # find out what line number the station was listed in, used later when we modify the file
	let "i += 1"
	done
}
...
local divider=" "
	...
	do
	line=$(grep -e "${sta[${i}]}" /tmp/stadump) # grab the line with the station bssid
	sed -i '/'"${sta[${i}]}"'/ s/.*/'"${line}\n        ip address:     \<b\>${sta_ip[${i}]}\<\/b\>\n        host name:      \<b\>${sta_hostname[${i}]}\<\/b\>\n        Karma SSID:     \<b\>${sta_ssid[${i}]}\<\/b\>"'/g' /tmp/stadump
